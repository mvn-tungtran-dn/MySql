...
select distinct blog.category_id from blog inner join news on blog.category_id=news.category_id where blog.is_active =1 or news.is_active=1;
...
SELECT title FROM baitap.blog where user_id not in (SELECT distinct user_id FROM baitap.comment where target_table="blog");
-- 13.Lấy 5 blog mới nhất và số lượng comment cho từng blog
select count(c.comment),c.target_id from (select * from blog order by id desc limit 5) as sub inner join comment as c on sub.id = c.target_id group by c.target_id;
...
select * from blog as b inner join user as u on b.user_id=u.id where u.is_active=1 order by b.id desc limit 10;
...
select u.* from user as u join (select * from blog order by id desc limit 2) as c on u.id=c.user_id;
-- 23 Lấy 2 blog, 2 news mà user có id = 1 đã comment
(select distinct target_id from comment where user_id = 1 and target_table = "news" limit 2)
union
(select distinct target_id from comment where user_id = 1 and target_table = "blog" limit 2); 
...
select to_user_id from follow where from_user_id = 1;
select * from comment order by id desc limit 1;
select * from (select to_user_id from follow where from_user_id = 1) as a cross join (select * from comment order by id desc limit 1) as b;
